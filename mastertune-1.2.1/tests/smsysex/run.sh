...
# Usage: ./run.sh /path/to/DelugeFirmware (checked out at mastertune-v7 or later)
# ARM=1 ./run.sh ...: built for the Deluge's Cortex-A9 with the firmware's flags and run in the emulator (see ../arm)
set -e
FW=$(cd "$1" && pwd)
...
cd "$B"
if [ -n "$ARM" ]; then
	# On the Cortex-A9, d_string.cpp is used as it is. ff.c reads unaligned words on purpose, as the Deluge allows.
	$ARM_CXX -fno-sanitize=alignment -I "$HERE/stubs" -I "$B" -I "$B/fatfs" -I "$FW/src/deluge" -I "$FW/src" -o harness \
	    "$HERE/harness.cpp" "$FW/src/deluge/storage/smsysex.cpp" "$FW/src/deluge/storage/smsysex_json.cpp" \
	    "$FW/src/deluge/util/d_string.cpp" fatfs/ff.c fatfs/ffunicode.c "$HERE/diskio_ram.c" "$FW/src/deluge/util/pack.c"
	RUN="$ARM_RUN" python3 "$HERE/client_test.py"
	exit
fi
